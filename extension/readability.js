...
Readability.prototype = {
  ...
  **/
  ...
  _getArticleTitleElement: function(curTitle){
          ...
          var titleMatchesAHeading = this._someNode(headings, function(heading) {                
                console.log("does %"+heading.innerText+"% include %"+curTitle+"%?")
                var h = heading.innerText.toLowerCase()                                                   
                if(h.includes(curTitle) || curTitle.includes(h) ){
                  console.log("yes, matching heading. Now steal styles", heading)
   ...
   * @return void
   **/
  _getArticleTitleStyle: function() {
    // var doc = this._doc;
    var doc = window.document
    var curTitle = "";
    var origTitle = "";
    ...
    try {
        ...
        if (typeof curTitle !== "string"){
          // curTitle = origTitle = this._getInnerText(doc.getElementsByTagName('title')[0]);
          curTitle = origTitle = document.querySelector("title").innerText
          }

          console.log("1 - curTitle is now", curTitle)
    } catch (e) {/* ignore exceptions setting the title. */}


    if (curTitle.match(/ [\|\-] /)) { // there is a - or an |
            console.log("match group 1")
            curTitle = curTitle.replace(/(\r\n|\n|\r)/gm,"") // linefeeds begone 
            curTitle = origTitle.replace(/(.*)[\|\-] .*/gi, '$1'); // remove it
            curTitle = origTitle.replace(/(.*)[\|\-] .*/gi, '$1'); // remove it again
            curTitle = curTitle.trim()            
            console.log("2 - curTitle is now", curTitle) // now clean title

            targetElement = this._getArticleTitleElement(curTitle)

            if (curTitle.split(' ').length < 3) // if less than 3 words
              curTitle = origTitle.replace(/[^\|\-]*[\|\-](.*)/gi, '$1'); // remove .. slmething    
    } else if (curTitle.indexOf(': ') !== -1) { // if there is not not : in the title
              // Check if we have an heading containing this exact string, so we
              // could assume it's the full title.
              console.log("match group 2")
              ...
              console.log("matched any hs?:", match)
              ...
              console.log("match group 3")

              var hOnes = doc.getElementsByTagName('h1');
              if (hOnes.length === 1){
                curTitle = this._getInnerText(hOnes[0]);
                targetElement = hOnes[0]
                }
    ...
    if(targetElement==null){ // still no result? go straight for the h with the matching title
      console.log("match group 4")             
            targetElement = this._getArticleTitleElement(curTitle)
    } 

    console.log("done matching groups")

    if (targetElement != null){
      console.log("we're at the end. This is the targetElement", targetElement)
      console.log("this iis the style it has", targetElement.style.fontFamily)
      computedStyles = window.getComputedStyle(targetElement)
      // computedStyles = targetElement.style.fontFamily
    } 
  ...
  **/
  ...
  _getBodyStyle: function(innerText) {
    var computedStyle;
    var allPs = document.querySelectorAll("p")

      allPs.forEach(function(element) {
            var p = element.innerText.substring(0,20).trim()
            var i = innerText.substring(0,20).trim()
            console.log("#gbs comparing p %"+p+"% to inner %"+i+"%")
            if(p.includes(i)){
              computedStyle = window.getComputedStyle(element)
              console.log("#gbs Got it. Fontfam is "+computedStyle.fontFamily)
            }          
      }, this);
  ...
  parse: function () {
    ...
    var contentStyle = this._getBodyStyle(textContent)

    // use the first 10 words of textcontent to find it in the window doc and grab its style
...
// console.log("REMOVE THIS DEBUG STUFF WHEN USING THE EXTENSION")
// var d = document
...
// wholePage._getArticleTitleStyle()